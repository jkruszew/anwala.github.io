...
def printclust(clust, labels=None, n=0):
  # indent to make a hierarchy layout
    for i in range(n):
        print(' '),
        print(' ', file=open('cluserASCII.txt', 'a+'))
    if clust.id < 0:
    ...
    # positive id means that this is an endpoint
        if labels == None:
            print(clust.id)
            print(clust.id, file=open('cluserASCII.txt', 'a+'))
        else:
            print(labels[clust.id])
            print(labels[clust.id], file=open('cluserASCII.txt', 'a+'))
...
def kcluster(rows, distance=pearson, k=4):
  ...
  # Create k randomly placed centroids
    ...
    for t in range(100):
        print('Iteration %d' % t)
        print('Iteration %d' % t, file = open("kcluster"+str(k)+".txt", 'a+'))
  ...
  # Randomly initialize the starting points of the locations in 2D
    ...
    # Move points
        ...
        # The error is percent difference between the distances
                #print(fakedist[j][k], " , ", realdist[j][k], " , ", fakedist[j][k] - realdist[j][k])
                if realdist[j][k] == 0:
                    realdist[j][k] = 0.1
